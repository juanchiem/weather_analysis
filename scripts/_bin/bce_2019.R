...
p2 <- p1 +
  ...
  # geom_line(Present, aes(x=julian, y=avg, group=1)) +
  geom_path(data=Present, aes(y = avg, color = "2019"), size = 1.2) #+

  # geom_vline(xintercept = 0, colour = "wheat4", linetype=1, size=1) +
  transition_states(avg, transition_length=0.5, state_length = 0.5) +
  shadow_mark()
