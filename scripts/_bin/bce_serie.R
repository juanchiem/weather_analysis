...
bce_71_17 <- bce_serie %>% # 46 años de datos
  filter(year!=2018) %>% 
  select(year, month, day, tmax:tmean) %>% 
  group_by(year) %>%
  mutate(newDay = seq(1, length(day))) %>% #dias 1:366 (el año no bisisesto será NA)         
  ungroup() %>% 
...
# construir la banda externa de valores de Tmean extremas de cada dia  
PastLows <- bce_71_17 %>%
  group_by(newDay) %>%
...
ggplot(aes(newDay, avg)) +
  ...
  geom_line(Past, mapping=aes(x=newDay, y=avg, group=1)) +
  geom_linerange(Past, mapping=aes(x=newDay, ymin=lower, ymax=upper), 
                 colour = "wheat4", alpha=.5)+
  geom_linerange(PastHighs, mapping=aes(x=newDay, ymin=upper, ymax=Pasthigh), 
                 colour = "red", alpha=.2)+
  geom_linerange(PastLows, mapping=aes(x=newDay, ymin=lower, ymax=Pastlow), 
                 colour = "blue", alpha=.2)->p0

  #geom_linerange(Past, mapping=aes(x=newDay, ymin=lower, ymax=upper), colour = "wheat4")
p0
